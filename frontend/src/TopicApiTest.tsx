// src/TopicApiTest.tsx
import React, { useState } from "react";
import { TOPICS, TopicConfig } from "./topic";
...
const TopicApiTest: React.FC = () => {
  ...
  const selectedTopic: TopicConfig | undefined = TOPICS.find(
    (t) => t.id === selectedId
  );
  ...
  async function handleFetch() {
    ...
    try {
      ...
      // Only send q if user typed something; otherwise Lambda will use defaultQuery
      if (customQuery.trim()) {
      ...
      const received: SearchItem[] = data.items || [];
      setItems(received);
    } catch (err: any) {
    ...
    >
      ...
      >
        <h1 style={{ margin: 0, fontSize: "1.7rem" }}>
          Topic API Test Console
        </h1>
        <p style={{ margin: "0.5rem 0 1.5rem", color: "#6b7280" }}>
          Choose a topic, optionally refine the search, and hit the Lambda to
          see live results.
        </p>
        ...
        <div>
          ...
          <p style={{ margin: "0 0 0.7rem", fontSize: "0.85rem", color: "#6b7280" }}>
          ...
          {items.length === 0 && !loading && !error && (
            <p style={{ fontSize: "0.9rem", color: "#6b7280" }}>
              No items yet. Try hitting &ldquo;Fetch results&rdquo; or changing
              the topic/query.
            </p>
          )}
          ...
          <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
            {items.map((item) => (
              ...
              >
                ...
                >
                  {item.source} {item.agency ? `· ${item.agency}` : ""}{" "}
